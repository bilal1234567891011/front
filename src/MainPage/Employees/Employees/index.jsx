...
const EmployeesRoute = ({ match }) => {
  ...
  const [filteredAttendanceState, setFilteredAttendanceState] = useState({state:false,fId:''});
return (
  <AttendanceTableContext.Provider value={{attendanceState, setAttendanceState,
   filteredAttendanceState, setFilteredAttendanceState}}>
  <Switch>
    <Redirect exact from={`${match.url}/`} to={`${match.url}/allemployees`} />
    <Route path={`${match.url}/allemployees`} component={AllEmployees} />
    <Route exact path={`${match.url}/addemployee`} component={AddEmployee} />
